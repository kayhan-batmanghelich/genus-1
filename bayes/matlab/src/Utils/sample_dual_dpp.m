...
for i = k:-1:1
  ...
  P = sum((B * V).^2,2);
  % added by Kayhan: begin
  P(Y(k:-1:(i+1))) = 0 ;  % previously selected items cannot be selected again. This value should be zero ut just tobe on the safe side
  % added by Kayhan: end
  P = P / sum(P);
...
if (length(Y) ~= length(unique(Y)))
    warning('||||||||||| There  is a redundacy in the set !!!  |||||||||||')
    Y = unique(Y) ;
end
% added by Kayhan: end
