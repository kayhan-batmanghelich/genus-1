...
Iternode = pe.Node(niu.IdentityInterface(
        fields = [ 'perplexity', 'learning_rate']),
    name = 'Iternode')

def clust_run(perplexity, learning_rate):
    """
    hopefully large scale visualization run
    """
    import numpy as np
    ...
    def tsne(data, n_comp, perplex, learn_r, niter):
    ...
    for name, group in groups:
        ...
        try:
            results[name] = tsne(data = model_data,
                                 n_comp = components,
                                 perplex = perplexity,
